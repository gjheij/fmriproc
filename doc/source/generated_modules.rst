...
.. code-block:: none
    ...
    Usage:
      spinoza_brainextraction [arguments] [options] <input dir> <skullstrip output> <mask output> <ants/FSL/cat12>
...
.. code-block:: none
    ...
    Usage:
      spinoza_extractregions [arguments] [options] <nighres folder> <probability folder> <ROI>
...
.. code-block:: none
    ...
    Arguments:
      -c <a,b,c,d>    list of values used for clipping of design matrix. Format must be [<top>,<bottom>,
      ...
      -n <session>    session ID (e.g., 1, 2, or none)
      -s <subject>    subject ID (e.g., 01). Can also be comma-separated list: 01,02,05
      -t <task ID>    If you have mutiple tasks specified in TASK_IDS or you just have multiple tasks and
...
.. code-block:: none
    ...
    Options:
      -o|--ow           only removes folders within single_subj workflow with "run-". If specific runs are
      ...
      --docker          run fmriprep through docker image (requires 'fmriprep-docker' to be installed!)
                        docker is not compatible with separate input folders for 'anat' and 'func', so the
                        input folder will be set to DIR_DATA_HOME by default. The T1w that is present in the
                        'anat'-folder will be used as input.
...
.. code-block:: none
    ...
    Options:
      --sge         submit individual NORDIC processes to the cluster for parallellization. If you do
                    this, it's advised to have identifiable 'run-' flags in your filenames so that the
                    template file is not overwritten; this can cause problems. If you do not have run
                    identifiers in your filenames, please run serially. This flag is inherited from
                    'master', so calling it there will pass on the flag here.
...
.. code-block:: none
    ...
    Options:
      ...
      --no-winsor         do not winsorize the image intensities of T1w (and T1map) from MPRAGE. Generally recommend doing so,
                          though..
...
.. code-block:: none
    ...
    Arguments:
      -s <subject>        subject ID (e.g., 01). Can also be comma-separated list: 01,02,05
      -n <session>        session ID (e.g., 1, 2, or none)
      -x <kwargs>         Additional commands to be passed to 'FAST'. Format should
                          be comma-separated flags as follows:
                            - if you specify a flag and values | <flag>=<value>
                            - if you specify a flag only | <flag>
...
.. code-block:: none
    ...
    Arguments:
      ...
      -j <cpus>           number of cores to use (default is 1)
      -q <queue>          submit jobs to a specific queue. Defaults to SGE_QUEUE_LONG in spinoza_setup
      -x <kwargs>         Additional commands to be passed to 'antsRegistration'. Format should
                          be comma-separated flags as follows:
                            - if you specify a flag and values | <flag>=<value>
                            - if you specify a flag only | <flag>
      ...
      -p <kwargs>         Additional commands to be passed to 'MASSP'. Format should
                          be comma-separated flags as follows:
                            - if you specify a flag and values | <flag>=<value>
                            - if you specify a flag only | <flag>
