...
the GLM. They can be supplied explicitly, for example::
    ...
    master -m 28 \
        -l 1 \
        -p /mnt/d/fMRI/SC2F \
        -x '--task SC2F --space T1w --contrast "USp;T;USp;1"'
...
``reg`` information that higher-level FSL tools expect.

Before running higher-level FEAT analyses, reconstruct this information with
``spinoza_featreg``.

For example, when the first-level BOLD data are already in T1w space and the
higher-level analysis should operate in MNI space::
...
* the space of the **first-level BOLD data**;
* the space required by the **higher-level analysis**.
...
The transformations themselves come from fMRIPrep. ``spinoza_featreg`` converts
or injects them into the representation expected by FEAT.

For example, native-space first-level data can be registered to MNI with::
...
For a T1w-space higher-level analysis::
...
The input-space flags are explicit because the first-level input space should
not have to be inferred exclusively from the name of the ``.feat`` directory.
...
For example::
    ...
    master -m 28 \
        ...
        -p /mnt/d/fMRI/SC2F \
        --local \
        -x '--contrast USp --registration-mode reg_standard'
...
Then combine runs within subjects for the contrast ``USp``::
