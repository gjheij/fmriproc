...
def ants_registration(fixed=None, moving=None, reg_type="rigid", output=None):
    ...
    python wrapper for call_antsregistration to perform registration with ANTs in the python environment. Requires the same
    input as call_antsregistration
    ...
    if fixed and moving:
        try:
            ...
            raise OSError("""Could not execute call_antsregistration; check your distribution or install the fmriproc
                          repository""")
...
def ants_applytrafo(
    ...
    fixed: str|nibabel.Nifti1Image
        string to nifti reference image (.nii.gz) or nibabel.Nifti1Image that will be con-verted temporarily to a file
        (fixed.nii.gz) in the working directory
    moving: str|nibabel.Nifti1Image
        moving (to-be-registered) image (.nii.gz) or nibabel.Nifti1Image that will be converted temporarily to a file
        (moving.nii.gz) in the working directory
    trafo: str|list
        list or single path to transformation files in order of application
    interp: str
        interpolation type: 'lin' (linear), 'nn' (NearestNeighbor), gau (Gaussian), bspl<order>, cws  CosineWindowedSinc), wws
        (WelchWindowedSinc), hws (HammingWindowed-Sinc), lws (LanczosWindowedSinc); default = 'nn'
    invert: int|list
        list or single integer with the length of trafo-list to specify which transformations to invert or not. Default = 0
        for all, meaning use as they are specified: do not invert
    output: str
    ...
    if invert != None:
        if isinstance(invert, list) and isinstance(trafo, list):
            if len(invert) != len(trafo):
                raise ValueError(f"list of inversion ({len(invert)}) does not equal list of transformations {len(trafo)}")
    ...
    try:
        ...
        raise OSError("""Could not execute call_antsapplytransforms; check your distribution or install the fmriproc
                      repository""")
...
def ants_applytopoints(chicken_file, output_file, trafo_file, invert=1):
    ...
    Wrapper for antsApplyTransformToPoints to warp a coordinate specified in `chicken_file` to a new space using `trafo_file`. Run :func:`fmriproc.utils.make_chicken_csv` and `call_antsregistration` from 1 space to another first.
...
def tkr2ctx(subject, coord=None):
    ...
    :func:`cxutils.pycortex.get_ctxsurfmove`)
...
def ctx2vert(surf,coord=None,rtol=0.015):
    ...
    Finds the closest vertices given an RAS coordinate as defined by pycortex. It uses np.closeall with a customizable
    rtol-value. If this function does not return anything, increase the rtol. If the original coordinate is not on the
    surface, results might differ.
    ...
    It runs through the surface-array and calculates the similarity between the RAS coordinates in there and the specified
    coordinate with a given tolerance. All elements of the coordinate should be as close to 1 as possible, showing more
    similarities between the given coordinate and the coordinate that has a vertex attached to it. Because of this iterative
    nature, the process takes a few seconds, so it's not that your system is slow per se.

    Returns a list of indices where the coordinate has passed the tolerance test. You can verify this by entering the indices
    in FreeView on the given surface. E.g., if you used lh.fiducial, enter the vertex in that box.
...
def fs2vert(subject, coord=None, hemi="lh"):
    ...
    dict
        ...
        * vert_obj (dict): output from :func:`cxutils.optimal.CalcBestVert.label_to_mask`, consisting of numpy.ndarrays
        representing a boolean mask of the vertex
    """
...
def ctx2tkr(subject, img=None, coord=None, correct=True, hm=True, ret=True, pad_ones=True):
    ...
    Convert a coordinate from Pycortex to FreeSurfer's TKR (surface) definition. Basically it adds the offset back
    (https://gallantlab.github.io/pycortex/_modules/cortex/freesurfer.html) that is added by Pycortex. With this particular
    function, you can apply that offset matrix to a given image, or enter a list of coordinate to which to apply the offset
    to. To just get the offset, specify the subject (needed for all operations), with 'hm' set to False (3x1 coordinate array)
    or True (4x4 homogenous matrix with offset in translation column).
    ...
    coord: list
        one or multiple coordinates to apply the offset to (e.g., left|right hemisphere) in list format (also when entering
        just 1 coordinate)
    correct: bool
        actually apply the matrix to an input image and return the nifti image with padded affine matrix. Should be used in
    ...
    ret: bool
        used in combination with the list of coordinates to return the corrected coordinates in a list. Artifact from when
        this was part of a class and included the corrected coordinates in the class object itself
    pad_ones: bool
    ...
    if len(ctx_offset) != 1:
        try:
            ...
            raise ValueError(f"""Got input with unhashable dimensions. Could either be a (3,) or (4,4) matrix, not
                             {ctx_offset.shape}""")
    else:
        offset = ctx_offset
...
def tkr2fs(subject, coord=None, fs_dir=None, pad_ones=True):
    ...
    Convert a coordinate from FreeSurfer's TKR (surface) definition to FreeSurfer's anatomical (volume) definition. This
    involves the procedure described here: https://surfer.nmr.mgh.harvard.edu/fswiki/CoordinateSystems scenario [3]: "I have a
    point on the surface ("Vertex RAS" in tksurfer) and want to compute the Scanner RAS in orig.mgz that corresponds to this
    point".
...
def rawavg2fs(subject, coord=None, fs_dir=None):
    ...
    Option [6] from https://surfer.nmr.mgh.harvard.edu/fswiki/CoordinateSystems: "*I have a CRS from a voxel in my functional/
    diffusion/ASL/rawavg/etc "mov" volume and want to compute the CRS for the corresponding point in the orig.mgz*"
...
def tkr2rawavg(subject,matrix=None,coord=None,reg=True,fs_dir=None, inv=False, out_type="voxel"):
    ...
    Computes the transformation from FreeSurfer TKR space (orig.mgz) to the native space (rawavg.nii.gz). It a ssumes the
    FreeSurfer and native space differ, which is not necessarily the case if you already have isotropic native data. You can
    either specify the registration file as per the output of tkregister2 or have this function create that file (register.dat
    by default)
    ...
    fs_dir: str
        `FreeSurfer` directory (default = SUBJECTS_DIR)
    inv: bool
        if the matrix file you specify with 'matrix' is actually mapping native to FreeSurfer, set this flag to True to invert
        the matrix
    out_type: str
...
def rawavg2lowres(fixed, moving, matrix, inv=False, out_file=None):
    ...
    :func:`fmriproc.transform.ants_applytransform`
    ...
    if inv == False:
        ...
        raise ValueError(f"""Unknown input {inv} for 'invert'-flag. Specify True (invert input matrix) or False (do not invert
                         input matrix)""")
...
def native_to_scanner(anat, coord, inv=False, addone=True):
    ...
    This function returns the RAS coordinates in scanner space given a coordinate in native anatomy space. Required inputs are
    an anatomical image to derive the VOX-to-RAS conversion from and a voxel coordinate. Conversely, if you have a RAS
    coordinate, you can set the 'inv' flag to True to get the voxel coordinate corresponding to that RAS-coordinate. To make
    the output 1x4, the addone-flag is set to true. Set to false if you'd like 1x3 coordinate.
    ...
    addone: bool
        False if you don't want a trailing *1*, returning a 1x3 array, or True if you want a trailing *1* to make a matrix
        homogenous

    Returns
    ----------
    numpy.ndarray
        (3,) or (4,) array containing the input coordinate `coord` in scanner convention (if `coord` is in voxel convention
        and `inv==False`) or voxel convention (if `coord` is in scanner convention and `inv==True`)
    ...
    >>> # ras2vox
    >>> native_to_scanner(
    >>>     'sub-001_space-ses1_hemi-L_vert-875.nii.gz',
    >>>     np.array([ -7.42000937, -92.96745521, -15.27866316]), 
    >>>     inv=True
    >>> )
    array([142,  48, 222,   1])
    >>> # disable trailing '1'
...
def mri_surf2surf(src_file=None, src_subj=None, trg_subj=None, out_file=None, hemi=None, return_file=False):
    ...
    Wrapped executable: mri_surf2surf. Transform a surface file from one subject to another via a spherical registration. Both
    the source and target subject must reside in your Subjects Directory, and they must have been processed with recon-all,
    ...
    >>> surf_file = mri_surf2surf(
    >>>     src_file="lh.V1_fsaverage",
    >>>     src_subj="fsaverage",
    >>>     trg_subj="sub-001",
    >>>     out_file="lh.V1_fsnative",
    >>>     hemi="lh"
    >>> )
    """
